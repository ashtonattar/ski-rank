...
- **2026-07-28 — Chat messages fixed to use atomic writes; `_saveToFirestore()` now merges instead of overwriting.** Found while testing the Friends tab: two users messaging within ~1s of each other could lose one or both messages, same root cause as the live-game race (see above), just affecting `messages` on the shared `state/global` doc instead.
  ...
  - **Not yet fixed / same risk elsewhere:** `friendRequests`, `challenges`, `pending`, `liveInvites`, explore-feed comments/likes, etc. are still written via the regular full-document save and are exposed to the identical race — they just haven't been proven to lose data the way messages were. If a similar bug is reported in one of those areas, the fix is the same pattern: targeted `arrayUnion`/dot-path update at the point of mutation, and exclude that field from the general save's `copy`.
...
| Riders tab — provisional badge missing | Riders with <5 games (`RATED_THRESHOLD`) show a full tier badge (e.g. "🎿 Developing") and no `*` on their rating, instead of "⏳ Provisional X/5" — because `renderPlayers()` calls `tier(p.rating)` / `ratingDisplay(p.rating)` without the `gamesPlayed` argument (index.html:7192, 7197). The Rankings tab passes `gamesPlayed` correctly and shows "⏳ 3/5" for the identical player. Same bug (missing `gamesPlayed` arg) also affects the player-picker dropdown (`tier(player.rating)`, line 6592) and the Friends sidebar (`tier(f.rating)`, line 11054). | Found 2026-07-28, not yet fixed |
...
---
*Last updated: 2026-07-28 (live-game refactor + setter-vote fix committed as `1dcc53d` and pushed, verified in both two-player and three-player judge mode; rating-engine upset multiplier made unconditional and unit-tested, not yet committed; Friends tab tested — found and fixed a chat-message data-loss race via `arrayUnion` + `merge:true` save, not yet committed; Riders tab tested — filters/friend-request/profile-link verified working, found two unfixed bugs: missing provisional badge, and seed-account stats inconsistent with Profile). Update this file after any further app changes so future sessions start with accurate context.*
